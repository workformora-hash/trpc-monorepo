...
class UserService {
  ...
  public async createUserWithEmailAndPassword(payload: CreateUserWithEmailAndPasswordInputType) {
    try {
      ...
      // check if user exist already or not (including soft-deleted users so they can re-register)
      const existingUser = await this.getUserByEmail(normalizedEmail, true);
      if (existingUser) {
        if (existingUser.deletedAt !== null || !existingUser.isActive) {
          // Purge all stale records associated with the soft-deleted user to prevent conflict
          await db.transaction(async (tx) => {
            await tx.delete(sessionsTable).where(eq(sessionsTable.userId, existingUser.id));
            await tx.delete(emailVerificationTokensTable).where(eq(emailVerificationTokensTable.userId, existingUser.id));
            await tx.delete(passwordResetTokensTable).where(eq(passwordResetTokensTable.userId, existingUser.id));
            await tx.delete(oauthAccountsTable).where(eq(oauthAccountsTable.userId, existingUser.id));
            await tx.delete(credentialsTable).where(eq(credentialsTable.userId, existingUser.id));
            await tx.delete(usersTable).where(eq(usersTable.id, existingUser.id));
          });
        } else {
  ...
  public async changePassword(
    ...
    this.validatePasswordSecurity(input.newPassword);
