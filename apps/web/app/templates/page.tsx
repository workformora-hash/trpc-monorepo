...
import { 
  Sparkles, Loader2, ClipboardCheck, ArrowRight, LayoutGrid, CheckCircle2, ChevronRight 
} from 'lucide-react';
...
export default function TemplatesPage() {
  ...
  const { data: templates = [], isLoading } = trpc.form.listFormTemplates.useQuery();
        ...
        {isLoading ? (
          ...
          <div className="grid md:grid-cols-3 gap-6">
            {templates.map((template) => (
              <div 
                key={template.id}
                className="bg-white dark:bg-neutral-900 border border-neutral-200 dark:border-neutral-800 rounded-2xl p-6 shadow-sm flex flex-col justify-between hover:shadow-md hover:border-neutral-350 dark:hover:border-neutral-700 transition-all space-y-6"
              >
                  ...
                  <div className="pt-2 border-t border-neutral-100 dark:border-neutral-800">
                    ...
                    <ul className="space-y-1.5 max-h-[140px] overflow-y-auto pr-1">
                      {template.fields.map((f, i) => (
                        <li key={i} className="text-[11px] text-neutral-500 dark:text-neutral-400 flex items-center gap-1.5">
